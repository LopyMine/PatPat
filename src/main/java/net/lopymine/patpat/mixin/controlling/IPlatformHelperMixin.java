...
public interface IPlatformHelperMixin {
	...
	private void handlePatPatKeybindingOnKeyPressed(NewKeyBindsScreen screen, Options options, /^? if >=1.21.9 {^//^int keyCode, int scanCode, int modifiers ^//^?} else {^/ int keyCode, int scanCode, int modifiers /^?}^/, CallbackInfo ci) {
		//? if >=1.21.9 {
