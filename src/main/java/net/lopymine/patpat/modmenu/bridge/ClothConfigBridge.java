...
public class ClothConfigBridge {
	...
	private static AbstractConfigListEntry<?> getOption(AbstractPatOption<?> option, ConfigEntryBuilder entryBuilder) {
		...
		return switch (option) {
			case SliderNumberOption<?> sliderNumberOption -> {
			...
			case NumberOption<?> numberOption -> {
			...
			case EnumOption enumOption -> {
				...
				yield enumBuilder.build();
			}
			default -> throw new IllegalStateException("Unexpected PatOption: " + option);
		};
