...
public class YACLConfigurationScreen {
	...
	public static Screen createScreen(Screen parent) {
		PatPatClientConfig defConfig = new PatPatClientConfig();
	...
	private static OptionGroup getVisualGroup(PatPatClientConfig defConfig, PatPatClientConfig config) {
		return SimpleGroupOptionBuilder.createBuilder("visual").options(collector -> collector.collect(
				...
				collector.addDoubleOptionAsSlider("hand_offset_x", -5D, 5D, 0.05D, defConfig.getAnimationOffsets().getOffsetX(), defConfig.getAnimationOffsets()::getOffsetX, defConfig.getAnimationOffsets()::setOffsetX),
				collector.addDoubleOptionAsSlider("hand_offset_y", -5D, 5D, 0.05D, defConfig.getAnimationOffsets().getOffsetY(), defConfig.getAnimationOffsets()::getOffsetY, defConfig.getAnimationOffsets()::setOffsetY),
				collector.addDoubleOptionAsSlider("hand_offset_z", -5D, 5D, 0.05D, defConfig.getAnimationOffsets().getOffsetZ(), defConfig.getAnimationOffsets()::getOffsetZ, defConfig.getAnimationOffsets()::setOffsetZ)
		)).build();
	}
