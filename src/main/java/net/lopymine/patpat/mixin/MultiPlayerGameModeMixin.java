...
public class MultiPlayerGameModeMixin {
	...
	private void patOnAttack(Player player, Entity entity, CallbackInfo ci) {
		if (!PatPatClientConfig.getInstance().getFunConfig().isPvpModeEnabled() || !PatPatClientConfig.getInstance().getMainConfig().isModEnabled()) {
			return;
		}
		...
		GameProfile profile = /*? if >=1.20.2 {*/ Minecraft.getInstance().getGameProfile(); /*?} else {*/ /*minecraft.getUser().getGameProfile(); *//*?}*/
